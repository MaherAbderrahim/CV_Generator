...
export const CreativeTemplate: React.FC<TemplateProps> = ({ data }) => {
          ...
          <div className="space-y-2.5 pt-1 text-slate-300">
            {section.items.map((item) => (
              <div key={item.id} className="text-xs space-y-0.5">
                ...
                {item.date && <p className="text-[10px] font-mono" style={{ color: accentColor }}>{item.date}</p>}
            ...
            section.items.map((item) => (
              <div key={item.id} className="section-item space-y-1.5 pb-3 border-b border-dashed border-slate-100 last:border-0 last:pb-0" data-section-id={section.id} data-item-id={item.id}>
                <div className="flex justify-between items-start">
                  ...
                  {item.date && (
                    <span className="text-[10px] font-mono shrink-0" style={{ color: accentColor }}>{item.date}</span>
                  )}
                </div>
  ...
  return (
    <div
      id="cv-capture-area"
      className={`a4-page cv-template-creative ${fontClass} ${fontSizeClass} ${lineHeightClass} flex flex-col`}
      style={{ backgroundColor: "#ffffff" }}
    >
      {/* 1. Header (Creative Layout Split) */}
      <div className="cv-creative-header w-full flex shrink-0 border-b-4 min-h-[140px] items-stretch" style={{ borderColor: accentColor }}>
        ...
        >
          ...
          <div
            className="absolute top-0 left-0 w-0 h-0 border-t-[30px] border-r-[30px] border-r-transparent opacity-85"
        ...
        >
          ...
          <div className="w-16 h-1 mt-3 rounded-full" style={{ backgroundColor: accentColor }} />
        </div>
      </div>
        ...
        >
          ...
          <div className="cv-header-block space-y-3">
            ...
            <div className="space-y-2 text-[11px] text-slate-300 font-medium">
              {personalInfo.email && (
                <div className="flex items-center gap-2">
                  <Mail size={11} className="shrink-0" style={{ color: accentColor }} />
              ...
              {personalInfo.website && (
                <div className="flex items-center gap-2">
                  <Globe size={11} className="shrink-0" style={{ color: accentColor }} />
            ...
            <div className="cv-header-block space-y-3">
              ...
              <div className="space-y-2 text-[11px] text-slate-300">
                {socialLinks.map((link) => (
                  ...
                  >
                    <span className="text-[9px] uppercase font-bold px-1 py-0.5 rounded leading-none shrink-0" style={{ color: accentColor, backgroundColor: `${accentColor}18` }}>
